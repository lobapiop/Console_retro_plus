# Project CRP – TEST-D01 – Luminosité

Version : 0.1  
Statut : réalisé 
Étude associée : ET001 – Module d’affichage  
Composant testé : Elecrow 5inch IPS HD Display-C 1024×600  
Référence attendue : DLH04950B  
Plateforme : Raspberry Pi 5 – 4 Go  
Système : Batocera  

---

## 1. Objectif

Évaluer la plage de luminosité et le confort dans plusieurs conditions d’éclairage.

## 2. Procédure

Tester :

1. luminosité minimale ;
2. luminosité à environ 50 % ;
3. luminosité maximale ;
4. pièce sombre ;
5. éclairage intérieur normal ;
6. proximité d’une fenêtre sans soleil direct.

## 3. Résultats

| Condition | Note sur 10 | Remarque |
|---|---:|---|
| Pièce sombre | 8 |  |
| Éclairage intérieur normal | 8 |  |
| Proximité d’une fenêtre | 8 |  |
| Luminosité minimale | 8|  |
| Luminosité maximale | 8 |  |

## 4. Contrôles complémentaires

| Contrôle | Résultat |
|---|---|
| Scintillement à faible luminosité | Non |
| Réglage suffisamment progressif | Oui |
| Mémorisation après arrêt | non testé |
| Confort à luminosité maximale | Ok |

## 5. Conclusion

Résultat : Réussi 

Réglage recommandé :
