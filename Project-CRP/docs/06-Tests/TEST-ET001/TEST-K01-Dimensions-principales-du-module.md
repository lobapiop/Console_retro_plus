# Project CRP – TEST-K01 – Dimensions principales du module

Version : 0.1  
Statut : à exécuter  
Étude associée : ET001 – Module d’affichage  
Composant testé : Elecrow 5inch IPS HD Display-C 1024×600  
Référence attendue : DLH04950B  
Plateforme : Raspberry Pi 5 – 4 Go  
Système : Batocera  

---

## 1. Objectif

Relever les dimensions nécessaires au futur gabarit FreeCAD.

## 2. Matériel nécessaire

- pied à coulisse recommandé ;
- règle de secours ;
- module non alimenté ;
- plan mécanique trouvé.

## 3. Précautions

- ne pas serrer fortement le pied à coulisse sur la dalle ;
- ne pas appuyer sur les nappes ;
- mesurer plusieurs fois ;
- noter l’incertitude de mesure.

## 4. Résultats

| Élément | Valeur mesurée | Valeur du plan trouvé | Écart |
|---|---:|---:|---:|
| Largeur hors tout | 122.77 | 121,11 mm | 1.66 |
| Hauteur hors tout | 95.07mm | 77,93 mm | 17.14 |
| Épaisseur de la dalle | 4.07 |  |  |
| Épaisseur du PCB | 7.87 |  |  |
| Profondeur maximale | 13.30 | 13,94 mm | -0.65 |
| Largeur de la zone active | 108.88 | 108 mm | 0.88 |
| Hauteur de la zone active | 66.36 | 64,8 mm | 1.56 |

Instrument utilisé : pied à coulisse

Incertitude estimée : épaisseur de PCB je n'ai pas pris en compte la mousse qui sépare la dalle du PCB.

## 5. Conclusion

Dimensions du plan confirmées : Partiellement

Remarques : voir Incertitudes
